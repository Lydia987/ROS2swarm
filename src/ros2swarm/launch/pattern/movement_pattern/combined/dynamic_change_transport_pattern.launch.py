...
def generate_launch_description():
    """Start the nodes required for the caging pattern."""
    ...
    ros2_pattern_node = launch_ros.actions.Node(
        package='ros2swarm',
        executable='dynamic_change_transport_pattern',
        namespace=robot_namespace,
        output='screen',
        parameters=[
            PathJoinSubstitution([config_dir, 'movement_pattern', 'combined', 'dynamic_change_transport_pattern.yaml'])],
    ...
    ros2_pattern_subnode_pushing = launch_ros.actions.Node(
        ...
        remappings=[
            (['/', robot_namespace, '/drive_command'],
             ['/', robot_namespace, '/drive_command_pushing_pattern'])
        ],
        parameters=[
            PathJoinSubstitution([config_dir, 'movement_pattern', 'combined', 'dynamic_change_transport_pattern.yaml'])],
        arguments=['--ros-args', '--log-level', log_level]
    )
    ld.add_action(ros2_pattern_subnode_pushing)

    ros2_pattern_subnode_caging = launch_ros.actions.Node(
        package='ros2swarm',
        executable='caging_pattern',
        namespace=robot_namespace,
        output='screen',
        remappings=[
            (['/', robot_namespace, '/drive_command'],
             ['/', robot_namespace, '/drive_command_caging_pattern'])
        ],
        parameters=[
            PathJoinSubstitution([config_dir, 'movement_pattern', 'combined', 'dynamic_change_transport_pattern.yaml'])],
        arguments=['--ros-args', '--log-level', log_level]
    )
    ld.add_action(ros2_pattern_subnode_caging)
